    ...
    def _train_epoch(self, data_loader, epoch):
        ...
        for i, (x_batch, y_batch) in enumerate(data_loader):
            ...
            if self.le_reg > 0:
                ...
                # LE  function can only compute single inputs, so we need to loop here
                loss_le = 0.
                penalties = []
            
                for input in x_batch:
                    les = LEs(input, self.model, compute_gradients=True, time_interval = self.time_interval) #compute_uv needed here for correct gradient computation
                    max_le = les[0]
                    max_le = F.relu(max_le - self.le_threshold) #only penalize if above threshold
                    penalties.append(max_le) #hinge loss
                    
                loss_le = self.le_reg * torch.stack(penalties).mean()
